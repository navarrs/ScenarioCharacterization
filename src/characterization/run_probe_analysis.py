...
from characterization.utils.viz.utils import plot_histograms_from_dataframe
...
def _build_probed_df(df: pd.DataFrame) -> pd.DataFrame:
...
def _plot_probe_outcome_pie(df: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    plt.close()
    _LOGGER.info("Saved probe outcome pie chart -> %s", out)


def _plot_score_distributions(df_probed: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    """
    if df_probed.empty:
        _LOGGER.warning("No probed scenarios — skipping score distributions plot.")
        return
    ...
    plot_histograms_from_dataframe(scores_df, str(out), dpi)
    _LOGGER.info("Saved score distributions -> %s", out)


def _plot_score_delta_density(df_probed: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    """
    if df_probed.empty:
        _LOGGER.warning("No probed scenarios — skipping score delta density plot.")
        return
    ...
    plt.savefig(out, dpi=dpi)
    plt.close()
    _LOGGER.info("Saved score delta density -> %s", out)


def _plot_score_delta_by_agent_type(df_probed: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    """
    if df_probed.empty:
        _LOGGER.warning("No probed scenarios — skipping score delta by agent type plot.")
        return
    ...
    plt.savefig(out, dpi=dpi)
    plt.close()
    _LOGGER.info("Saved score delta by agent type -> %s", out)


def _plot_score_scatter(df_probed: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    """
    if df_probed.empty:
        _LOGGER.warning("No probed scenarios — skipping score scatter plot.")
        return
    ...
    plt.savefig(out, dpi=dpi)
    plt.close()
    _LOGGER.info("Saved score scatter -> %s", out)


def _plot_affected_agents_histogram(df_probed: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    """
    if df_probed.empty:
        _LOGGER.warning("No probed scenarios — skipping affected agents histogram.")
        return
    ...
    plt.savefig(out, dpi=dpi)
    plt.close()
    _LOGGER.info("Saved affected agents histogram -> %s", out)
...
def _save_summary_json(df: pd.DataFrame, df_probed: pd.DataFrame, output_dir: Path) -> None:
    ...
    _LOGGER.info("Saved probe analysis summary -> %s", out)
...
def run(cfg: DictConfig) -> None:
    ...
    _plot_probe_outcome_pie(df, output_dir, dpi)
    _plot_score_distributions(df_probed, output_dir, dpi)
    _plot_score_delta_density(df_probed, output_dir, dpi)
    _plot_score_delta_by_agent_type(df_probed, output_dir, dpi)
    _plot_score_scatter(df_probed, output_dir, dpi)
    _plot_affected_agents_histogram(df_probed, output_dir, dpi)
    _save_summary_json(df, df_probed, output_dir)
