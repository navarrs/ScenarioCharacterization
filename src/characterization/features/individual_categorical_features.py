...
class IndividualCategoricalFeatures(BaseFeature):
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        with open(config.vehicle_percentiles_file) as f:
            self.vehicle_percentiles = json.load(f)
        with open(config.cyclist_percentiles_file) as f:
            self.cyclist_percentiles = json.load(f)
        with open(config.pedestrian_percentiles_file) as f:
            self.pedestrian_percentiles = json.load(f)
    ...
    def categorize(self, value: float, agent_type: AgentType, feature_name: str) -> float:
        ...
        match agent_type:
            case AgentType.TYPE_VEHICLE | AgentType.TYPE_EGO_VEHICLE:
                percentiles = self.vehicle_percentiles.get(feature_name, [])
            case AgentType.TYPE_CYCLIST:
                percentiles = self.cyclist_percentiles.get(feature_name, [])
            case AgentType.TYPE_PEDESTRIAN:
                percentiles = self.pedestrian_percentiles.get(feature_name, [])
            case _:
                logger.warning("Unknown agent type: %s. Returning original value.", agent_type)
                return -1.0

        for num_percentile, threshold in enumerate(percentiles):
            if value <= float(threshold):
                return float(num_percentile + 1)  # Categories start from 1
        return float(len(percentiles) + 1)  # Above highest percentile

    def compute_individual_features(self, scenario: Scenario, return_criterion: ReturnCriterion) -> Individual:
        ...
        for n in range(agent_data.num_agents):
            ...
            scenario_speeds.append(self.categorize(speed, agent_type, "speed"))
            scenario_speed_limit_diffs.append(self.categorize(speed_limit_diff, agent_type, "speed_limit_diff"))
            scenario_accelerations.append(self.categorize(acceleration, agent_type, "acceleration"))
            scenario_decelerations.append(self.categorize(deceleration, agent_type, "deceleration"))
            scenario_jerks.append(self.categorize(jerk, agent_type, "jerk") if jerk is not None else -1.0)
            scenario_waiting_periods.append(self.categorize(waiting_period, agent_type, "waiting_period"))
            ...
            scenario_kalman_difficulties.append(self.categorize(kalman_difficulty, agent_type, "kalman_difficulty"))
    ...
    def compute(self, scenario: Scenario) -> ScenarioFeatures:
        ...
        return ScenarioFeatures(
            metadata=scenario.metadata,
            individual_features=self.compute_individual_features(scenario, self.return_criterion),
