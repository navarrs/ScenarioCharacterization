"""Aviation-domain scoring functions for individual agents and interaction pairs."""

import numpy as np

from characterization.domains.aviation.features.individual_utils import TrajectoryType
...
TRAJECTORY_TYPE_WEIGHTS: dict[str, float] = {
    TrajectoryType.TYPE_STATIONARY: 8.0,
    TrajectoryType.TYPE_STRAIGHT: 0.5,
    TrajectoryType.TYPE_TURNING: 1.0,
    TrajectoryType.TYPE_U_TURN: 1.0,
}
...
def simple_individual_score(
    ...
    trajectory_type: str = TrajectoryType.TYPE_STATIONARY,
    ...
    """Aggregate a simple score for an aviation agent from weighted feature values.
    ...
    Args:
        ...
        trajectory_type: Trajectory type string.
...
def simple_interaction_score(
    ...
    """Aggregate a simple score for an aviation agent pair from weighted interaction feature values.
...
def compute_imputation_score(agent_trajectory: RawAgentTrajectory) -> float:
    """Compute an imputation score (count of invalid frames) for the given agent trajectory.

    Args:
        agent_trajectory: Agent trajectory data.
    ...
    if agent_trajectory.num_frames == 0:
        return 0.0
    return float(np.sum(agent_trajectory.valid == INVALID_STATE_VALUE))
...
def compute_kalman_score(
    ...
    Returns:
        Normalized difficulty score. Returns 0.0 if insufficient data.
    """
    ...
    predicted_position = estimate_kalman_filter(
        past_positions,
        last_valid_future_index + 1,
        min_valid_points=min_past_valid_points,
        frame_indices=past_indices,
    )
...
def compute_simple_agent_score(
    ...
    *,
    ...
    """Compute a simple score based on imputation quality and Kalman difficulty.
    ...
    Returns:
        Computed score; -1.0 if the agent data is invalid.
    """
    if not is_valid:
        return -1.0
...
SCORE_FUNCTIONS_REGISTRY: dict[str, object] = {"simple": compute_simple_agent_score}
