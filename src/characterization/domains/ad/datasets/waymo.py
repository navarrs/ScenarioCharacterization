...
from characterization.domains.ad.datasets.base_dataset import BaseDataset
from characterization.domains.ad.schemas.scenario import (
    AgentData,
    AgentType,
    DynamicMapData,
    Scenario,
...
class WaymoData(BaseDataset):
    ...
    def transform_scenario_data(self, scenario_data: dict[str, Any]) -> Scenario:
        ...
        ego_agent_index = scenario_data["sdc_track_index"]
        tracks_to_predict = scenario_data["tracks_to_predict"]
        tracks_to_predict_index = np.asarray(tracks_to_predict["track_index"] + [ego_agent_index])
