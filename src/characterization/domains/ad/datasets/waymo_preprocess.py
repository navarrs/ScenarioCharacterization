...
def decode_tracks_from_proto(tracks: Iterable[scenario_pb2.Track]) -> dict[str, Any]:
...
def decode_map_features_from_proto(map_features: Iterable[scenario_pb2.MapFeature]) -> dict[str, Any]:
    ...
    for cur_data in map_features:
        cur_info: dict[str, Any] = {"id": cur_data.id}
    ...
    if len(polylines_list) == 0:
        logger.warning("No polylines found in the map features.")
        return map_infos
...
def decode_dynamic_map_states_from_proto(dynamic_map_states: Iterable[scenario_pb2.DynamicMapState]) -> dict[str, Any]:
...
def process_waymo_data_with_scenario_proto(data_file: str, output_path: str | None = None) -> list[dict[str, Any]]:
    ...
    ret_infos = []
    for _cnt, data in enumerate(dataset):
        info: dict[str, Any] = {}
...
def create_infos_from_protos(raw_data_path: str, output_path: str, num_workers: int = 8) -> None:
    ...
    logger.info("Waymo info train file is saved to %s", sample_filename)
