"""Domain-agnostic counterfactual scenario probing runner.
...
Example usage:
    Run aviation probing with default config:
        uv run -m characterization.run_scenario_probing domain=aviation

    Run AD probing:
        uv run -m characterization.run_scenario_probing domain=ad scenarios_dir=data/my_ad_scenarios \
            maps_dir=null viz_dir=null

    Increase parallelism and disable visualization:
        uv run -m characterization.run_scenario_probing num_workers=4 viz_dir=null
"""
...
from characterization.domains.ad.probing import CounterfactualProber as ADProber
from characterization.domains.ad.schemas.scenario import Scenario as ADScenario
from characterization.domains.aviation.probing import CounterfactualProber as AviationProber
...
from characterization.domains.aviation.utils.scenario_characterization_utils import visualize_scenario
...
def _print_probe_summary(scenario_id: str, airport_id: str, probe: CriticalProbe | None) -> None:
    if probe is None:
        print(f"  [{airport_id}/{scenario_id}] no impactful probe found")  # noqa: T201
        return
    ego_label = "ego" if probe.is_ego_agent else "other"
    delta = probe.score_after - probe.score_before
    print(  # noqa: T201
        f"  [{airport_id}/{scenario_id}] agent {probe.probed_agent_id} ({ego_label})"
        f"  before={probe.score_before:.4f}"
        f"  after={probe.score_after:.4f}"
        f"  delta={delta:+.4f}"
        f"  affected={probe.affected_agent_ids}"
        f"  crit_ts={probe.criticality_results}",
    )


def _save_probe_json(probe: CriticalProbe, probe_path: Path) -> None:
...
def _process_one_aviation(
    ...
    scenario = load_scenario(pkl_path, maps_dir, airport_id, map_cache)
    if scenario is None:
        return None

    prober = AviationProber(probing_cfg)
    try:
    ...
    row = _probe_to_csv_row("aviation", airport_id, event_subfolder, scenario.metadata.scenario_id, probe)
    ...
    if probe is not None:
        ...
        if probes_dir is not None:
            _save_probe_json(probe, probes_dir / relative.with_suffix(".json"))
        if viz_dir is not None:
            visualize_scenario(scenario, airport_id, viz_dir, viz_cache, cfg)
...
def _process_one_ad(
    ...
    try:
        scenario = ADScenario.from_pickle(pkl_path)
    except Exception:
        _LOGGER.exception("Failed to load AD scenario from %s", pkl_path)
        return None

    prober = ADProber(probing_cfg)
    try:
        probe = prober.probe_scenario(scenario)
    except Exception:
        _LOGGER.exception("Failed to probe AD scenario %s", pkl_path)
        return None

    scenario_id = scenario.metadata.scenario_id
    event_subfolder = _event_subfolder_from_path(pkl_path, scenarios_dir)
    row = _probe_to_csv_row("ad", "", event_subfolder, scenario_id, probe)

    if probe is not None:
        scenario.critical_probe = probe
        _print_probe_summary(scenario_id, "ad", probe)
        relative = pkl_path.relative_to(scenarios_dir)
        out_path = output_dir / relative
        out_path.parent.mkdir(parents=True, exist_ok=True)
        scenario.to_pickle(out_path.parent)
...
def _run_serial_aviation(
    ...
    for pkl_path in pkl_files:
        ...
        event_subfolder = _event_subfolder_from_path(pkl_path, scenarios_dir)
        csv_rows.append(
            _probe_to_csv_row("aviation", airport_id, event_subfolder, scenario.metadata.scenario_id, probe)
        )
        if probe is None:
...
def _run_serial_ad(
    ...
    for pkl_path in pkl_files:
        ...
        scenario_id = scenario.metadata.scenario_id
        event_subfolder = _event_subfolder_from_path(pkl_path, scenarios_dir)
        csv_rows.append(_probe_to_csv_row("ad", "", event_subfolder, scenario_id, probe))
        if probe is None:
            _LOGGER.info("No useful probe found for scenario %s", pkl_path)
            continue
...
@hydra.main(config_path="config", config_name="scenario_probing", version_base=None)
def main(cfg: DictConfig) -> None:
    """Load every scenario under ``cfg.scenarios_dir``, apply counterfactual probing, and write results.

    Dispatches to domain-specific loading and probing logic based on ``cfg.domain``.

    Args:
        cfg: Hydra configuration. See ``config/scenario_probing.yaml`` for all parameters.
    """
    domain: str = cfg.domain
    if domain not in ("aviation", "ad"):
        msg = f"Unknown domain '{domain}'. Valid values: 'aviation', 'ad'."
        raise ValueError(msg)

    scenarios_dir = Path(cfg.scenarios_dir)
    output_dir = Path(cfg.output_dir)
    maps_dir = Path(cfg.maps_dir) if cfg.get("maps_dir") else None
    viz_dir = Path(cfg.viz_dir) if cfg.get("viz_dir") else None
    probes_dir = Path(cfg.probes_dir) if cfg.get("probes_dir") else None
    ...
    _LOGGER.info("Found %d scenario(s) in %s (domain=%s)", len(pkl_files), scenarios_dir, domain)

    probing_cfg_dict = OmegaConf.to_container(cfg.probing, resolve=True)
    if num_workers > 1:
        probing_cfg_dict["n_jobs"] = 1  # pyright: ignore[reportCallIssue,reportArgumentType,reportOptionalSubscript,reportIndexIssue]
    ...
    if num_workers > 1:
        if domain == "aviation":
            raw_results: list[dict[str, str] | None] = Parallel(n_jobs=num_workers)(
                delayed(_process_one_aviation)(
                    p, maps_dir, output_dir, probes_dir, viz_dir, scenarios_dir, probing_cfg, cfg
                )
                for p in pkl_files
            )  # pyright: ignore[reportAssignmentType]
        else:
            raw_results = Parallel(n_jobs=num_workers)(
                delayed(_process_one_ad)(p, output_dir, probes_dir, scenarios_dir, probing_cfg) for p in pkl_files
            )  # pyright: ignore[reportAssignmentType]
        csv_rows = [r for r in raw_results if r is not None]
        ok = sum(1 for r in csv_rows if r["probe_found"] == "yes")
    elif domain == "aviation":
        csv_rows, ok = _run_serial_aviation(
            pkl_files, maps_dir, output_dir, probes_dir, viz_dir, scenarios_dir, probing_cfg, cfg
        )
    else:
        csv_rows, ok = _run_serial_ad(pkl_files, output_dir, probes_dir, scenarios_dir, probing_cfg)
