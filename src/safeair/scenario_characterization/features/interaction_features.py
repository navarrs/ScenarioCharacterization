...
from safeair.scenario_characterization.features import interaction_utils
...
class InteractionFeatures(BaseFeature):
    ...
    def _get_candidate_pairs(self, scenario: Scenario) -> list[tuple[int, int]]:
        ...
        for i, j in itertools.combinations(range(n), 2):
            ...
            vx_i, vy_i = interaction_utils.velocity_components_2d(
                speeds[i][joint][fov_mask], headings[i][joint][fov_mask]
            )
            vx_j, vy_j = interaction_utils.velocity_components_2d(
                speeds[j][joint][fov_mask], headings[j][joint][fov_mask]
            )
    ...
    def _compute_pair(self, scenario: Scenario, idx_a: int, idx_b: int) -> InteractionPairFeatures:
        ...
        ttc = interaction_utils.compute_ttc(
            pos_a, pos_b, speeds_a, speeds_b, headings_a, headings_b, joint_valid_gated, self.return_criterion
        )
