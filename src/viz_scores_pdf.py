...
def run(cfg: DictConfig) -> None:
    ...
    for score_type, score_type_path in cfg.score_types.items():
        ...
        scores[score_type] = [float(s[list(s.keys())[0]]['scene_score']) for s in scores_cache]
    ...
    for key in scores_df.keys():
        ...
        plt.tight_layout()
        plt.legend()
        plt.savefig(os.path.join(cfg.output_dir, f'{key}_kde.png'), bbox_inches='tight', dpi=300)
        plt.close()
